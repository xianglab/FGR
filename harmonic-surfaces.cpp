...
const double eta =0.2; //0.2,1,5;
const int MCN = 50000;//Monte Carlo sampling rate
string idstr="Gau_b1e1_5e2_";
//*********** **************** *********

//change if using different spectral density(change also in main())
const double omega_max = 2.5; //20; //20 for ohmic, 2.5 for gaussian
const double d_omega = 0.025; //0.1 for ohmic, 0.025 for gaussian
const int LEN = 1024; //t choices in FFT, 512 for ohmic, 1024 for gausian
const double DeltaT=0.3; //FFT time sampling interval, 0.2 for ohmic, 0.3 for gaussian
const double hbar = 1;
const double DAcoupling = 0.1;
const double sigma = 0.1; //for gaussian spectral density
const double omega_op = 1.0; //for gaussian spectral density
...
const double T0= -DeltaT*LEN/2; //-153.6; //FFT time sampling starting point
...
int main (int argc, char *argv[]) {
    ...
    double *cuu_eq_D_forward_ana = new double [NMD]; //for 2nd cumu analytical
    
    for (i=0; i< NMD; i++) cuu_eq_D_forward[i] = cuu_eq_D_forward_ana[i] = 0;
    ...
    for (i=0; i< NMD/STEPS; i++) outfile << cuu[i] << endl;
    outfile.close();
    outfile.clear();

    outfile.open((emptystr+"cuu_D_"+idstr+".dat").c_str());
    for (i=0; i< NMD/STEPS; i++) outfile << cuu_D[i] << endl;
    outfile.close();
    outfile.clear();
    
    outfile.open((emptystr+"cuu_eq_"+idstr+".dat").c_str());
    for (i=0; i< NMD/STEPS; i++) outfile << cuu_eq[i] << endl;
    outfile.close();
    outfile.clear();
    
    outfile.open((emptystr+"cuu_eq_D_"+idstr+".dat").c_str());
    for (i=0; i< NMD/STEPS; i++) outfile << cuu_eq_D[i] << endl;
    outfile.close();
    outfile.clear();
    ...
    */
    ...
    cout << "ducl0_avg_ana = " << ducl0_avg_ana << endl;
    ...
    for (i=0; i< LEN; i++) {
        if (t_array[i] >= 0) {
            ...
            MDlen = static_cast<int>(abs(t_array[i] / DT));
            for (j=0; j< MDlen; j++) integrand_marcus[j] = (t_array[i] + j*DT) * cuu_eq_D_forward_ana[j];//cuu_eq_D_backward_ana[j];
            integral = Integrate(integrand_marcus, MDlen, -DT); //check ABSDT or DT?
            corr1[i] = cos(t_array[i]*ducl0_avg_ana/hbar) * exp(-integral/hbar/hbar);
            corr2[i] = sin(t_array[i]*ducl0_avg_ana/hbar) * exp(-integral/hbar/hbar);
        }
