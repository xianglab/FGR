...
string molecule = "MIA"; //Fulvene, BMA or MIA
const int n_omega = 96;//78; //number of normal modes, Fulvene=30; BMA=78; MIA=96
double Delta_11 = 0; //donor
double Delta_22 = 2.4615e-03;//2.9255e-02; //acceptor. Fulvene=-0.98911; BMA=2.9255e-02; MIA=2.4615e-03(reversed)
const double fs2au = 41.341105; //1 fs = 41.34 a.u.
const double kT2au = 3.168429139e-6;// kT(1 Kelvin) = 3.2e-6 a.u.

double temperature = 200; //K
const int LEN = 1024; //FFT time steps
double DeltaT_fs = 0.18;
...
int main (int argc, char *argv[]) {
    ...
    */
    ...
    for (i = 0; i < LEN; i++) {
        t = T0 + DeltaT * i;
        corr1[i] *= exp(-t*t / gdamp);// exp(- abs(t) / tdamp);
        corr2[i] *= exp(-t*t / gdamp);//exp(- abs(t) / tdamp);
    }
    
    outfile.open((emptystr + "Exact_EFGRL_tre_damp_" + nameapp + ".dat").c_str());
    for(i=0; i<nn; i++) outfile << corr1[i] << endl;
    outfile.close();
    outfile.clear();
    
    outfile.open((emptystr + "Exact_EFGRL_tim_damp_" + nameapp + ".dat").c_str());
    for(i=0; i<nn; i++) outfile << corr2[i] << endl;
    outfile.close();
    outfile.clear();
     */
    ...
    for (i = 0; i < LEN; i++) {
        t = T0 + DeltaT * i;
        corr1[i] *= exp(-t*t / gdamp);// exp(- abs(t) / tdamp);exp(- abs(t) / tdamp* 1.7);
        corr2[i] *= exp(-t*t / gdamp);// exp(- abs(t) / tdamp);exp(- abs(t) / tdamp* 1.7);
    }
     */
    ...
    for (i = 0; i < LEN; i++) {
        t = T0 + DeltaT * i;
        corr1[i] *= exp(-t*t / gdamp * 2.5);// exp(- abs(t) / tdamp);exp(- abs(t) / tdamp * 2.4);
        corr2[i] *= exp(-t*t / gdamp * 2.5);// exp(- abs(t) / tdamp);exp(- abs(t) / tdamp * 2.4);
    }
