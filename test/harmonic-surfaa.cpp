...
const int MCN = 100000;//1000000; //Monte Carlo sampling number
...
int main (int argc, char *argv[]) {
    ...
    double mc_re_ana[LEN];
    double mc_im_ana[LEN];
    ...
    for (j=0; j < MCN; j++) { //Monte Carlo phase-space integration (R,P)
        ...
        for (i=0; i< LEN; i++) {
            if (t_array[i] >= 0) {
                ...
                integral_du_ana[i] = 0;
                for (w=1; w<n_omega; w++) integral_du_ana[i] += req[w]*omega_array[w]*(R0[w]-req[w]/2)*sin(omega_array[w]*t_array[i]) + V0[w]*req[w]*(1-cos(omega_array[w]*t_array[i]));
                mc_re_ana[i] += cos(integral_du_ana[i]/hbar);
                mc_im_ana[i] += sin(integral_du_ana[i]/hbar);
            }
        }
    ...
    //Analyze Analytical dynamics on average surface + wigner sampling
    for (i=0; i< LEN; i++) { //Monte Carlo averaging
        mc_re_ana[i] /= MCN;
        mc_im_ana[i] /= MCN;
        corr1[i] = mc_re_ana[i]; //  k(t) re
        corr2[i] = mc_im_ana[i]; //  k(t) im
    }
    
    outfile.open((emptystr+"nlsc_ana_t_re_"+idstr+".dat").c_str());
    for (i=0; i< LEN; i++) outfile << corr1[i] << endl;
    outfile.close();
    outfile.clear();
    
    outfile.open((emptystr+"nlsc_ana_t_im_"+idstr+".dat").c_str());
    for (i=0; i< LEN; i++) outfile << corr2[i] << endl;
    outfile.close();
    outfile.clear();
    ...
    outfile.open((emptystr+"nlsc_ana_re_"+idstr+".dat").c_str());
    for (i=0; i<nn/2; i++) outfile << corr1_orig[i]*LEN*DeltaT*DAcoupling*DAcoupling << endl;
    outfile.close();
    outfile.clear();
    
    outfile.open((emptystr+"nlsc_ana_im_"+idstr+".dat").c_str());
    for (i=0; i<nn/2; i++) outfile << corr2_orig[i]*LEN*DeltaT*DAcoupling*DAcoupling << endl;
    outfile.close();
    outfile.clear();
    
    //Analyze dynamics on average surface + wigner sampling
    for (i=0; i< LEN; i++) { //Monte Carlo averaging
        mc_re[i] /= MCN;
        mc_im[i] /= MCN;
