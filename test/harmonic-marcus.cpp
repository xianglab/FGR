...
int main (int argc, char *argv[]) {
    ...
    double *cuu_eq_D_forward_ana = new double [NMD_forward]; //for 2nd cumu
    double *cuu_eq_D_backward_ana = new double [NMD_backward];
    for (i=0; i< NMD_forward; i++) cuu_eq_D_forward[i] = cuu_eq_D_forward_ana[i] = 0;
    for (i=0; i< NMD_backward; i++) cuu_eq_D_backward[i] = cuu_eq_D_backward_ana[i] = 0;
 ...
 */       
        ...
        for (tau =0 ; tau< NMD; tau++) {
            ...
            propagate_donor(Rcl0, Vcl0, DT*tau, omega_array, Rt);//analytical
            cuu_eq_D_forward_ana[tau] += ducl0 * DU(Rt, omega_array, req);
