...
const int STEPS = 10;//50; //for correlation function skip steps
...
int main (int argc, char *argv[]) {
    ...
    double *cuu_ana = new double [LENMD/STEPS]; //noneq <du(0) du(t)>_donor
    ...
    ducl0_avg /= MCN; // <du>_D numerical

    double ducl0_avg_ana(0);
    for (i=1;i<N;i++) ducl0_avg_ana += -0.5*omega_array[i]*omega_array[i]*req[i]*req[i];
    
    for (tau=0; tau< NMD_forward ; tau++){
        if (tau%STEPS==0) for (w=1; w<N; w++) cuu_ana[tau/STEPS] += omega_array[w]*omega_array[w]*req[w]*req[w]*cos(omega_array[w]*tau*DT)/beta;
    }
    
    //<du(0) du(t)_D/avg>_D for average- and donor-surface dynamics
    cout << "Length of Cuu = " << NMD_forward << endl;
    ...
    for (i=0; i< NMD_forward; i++) outfile << cuu_eq_D[i] << endl;
    outfile.close();
    outfile.clear();
    
    outfile.open((emptystr+"Cuu_ana_"+idstr+".dat").c_str());
    for (i=0; i< NMD_forward; i++) outfile << cuu_ana[i] - ducl0_avg_ana*ducl0_avg_ana << endl;
    outfile.close();
    outfile.clear();
