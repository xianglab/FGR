...
const double omega_max = 2.5;// 2.5; //20;
const double d_omega = 0.025;//0.025;
...
const double DeltaT=0.3;//0.3; //FFT time sampling interval
...
const int MCN = 10000;//5000000; //Monte Carlo sampling number
...
int main (int argc, char *argv[]) {
    ...
    stringstream ss;
    string emptystr("");
    string idstr="Gau_5sigma_b0.5_1e5_";
    double omega;
...
*/    
    ...
    int rare(0); //rare event count, beyond 5 sigma
    double random_num;
    ...
    for (j=0; j < MCN; j++) { //Monte Carlo phase-space integration (R,P)
        ...
        for (w=1;w < n_omega; w++) {
            random_num = GAUSS(&seed);
            while ( abs(random_num) > 5.0 ) {
                random_num = GAUSS(&seed);
                rare++;
            }
            R0[w] = R[w] = random_num * sigma_x[w];//initial conf sampling
            random_num = GAUSS(&seed);
            while ( abs(random_num) > 5.0 ) {
                random_num = GAUSS(&seed);
                rare++;
            }
            V0[w] = V[w] = random_num * sigma_p[w];//initial momentum sampling
        }
