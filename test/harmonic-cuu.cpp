...
const double omega_max = 2.5;//2.5; //20; //20 for ohmic, 2.5 for gaussian
const double d_omega = 0.025;
const int n_omega = static_cast<int>(omega_max / d_omega);

const int MCN = 10;//10000;//1000000; //Monte Carlo sampling number

const int N = n_omega; //number of degrees of freedom
const int LEN = 1024; //number of t choices in FFT
const int STEPS = 1;//50; //for correlation function skip steps
...
void CorrFunc(const int trun, const int tcor, double A[], double Corr[]);
...
int main (int argc, char *argv[]) {
    ...
    */
    ...
    double *cuu_ana = new double [LENMD]; //noneq <du(0) du(t)>_donor
    
    double *cuu_eq_D = new double [LENMD]; //eq <du(0) du(t)>_donor
    double *cuu_eq_D_accum = new double [LENMD]; //eq <du(0) du(t)>_donor
    ...
    req[0]=sigma_x_cl[0]=sigma_p_cl[0]=0;
    ...
    for (i=0; i<LENMD; i++) cuu_ana[i] = cuu_eq_D[i] =  cuu_eq_D_accum[i] = 0;
    
    int NMD_forward = static_cast<int>(abs(t_array[LEN-1]/DT));
    int NMD_backward = static_cast<int>(abs(t_array[0]/DT));
    int LEN_TRAJ_EQ = NMD_forward*20;
    
    double *du_accum = new double [LEN_TRAJ_EQ]; // for equilibrium Cuu(t)

    double *cuu_eq_D_forward = new double [NMD_forward]; //for 2nd cumu
    double *cuu_eq_D_backward = new double [NMD_backward];
    double *cuu_eq_D_forward_ana = new double [NMD_forward]; //for 2nd cumu analytical
    double *cuu_eq_D_backward_ana = new double [NMD_backward];
    double *cuu_eq_D_forward_ana1 = new double [NMD_forward]; //for 2nd cumu analytical
    double *cuu_eq_D_backward_ana1 = new double [NMD_backward];
    double *cuu_eq_D_forward_ana2 = new double [NMD_forward]; //for 2nd cumu analytical
    double *cuu_eq_D_backward_ana2 = new double [NMD_backward];
    double *cuu_eq_D_forward_ana3 = new double [NMD_forward]; //for 2nd cumu analytical
    double *cuu_eq_D_backward_ana3 = new double [NMD_backward];
    double *cuu_eq_D_forward_ana4 = new double [NMD_forward]; //for 2nd cumu analytical
    double *cuu_eq_D_backward_ana4 = new double [NMD_backward];
    
    for (i=0; i< NMD_forward; i++) cuu_eq_D_forward[i] = cuu_eq_D_forward_ana[i] = cuu_eq_D_forward_ana1[i] =cuu_eq_D_forward_ana2[i] = cuu_eq_D_forward_ana3[i] = cuu_eq_D_forward_ana4[i] =0;
    for (i=0; i< NMD_backward; i++) cuu_eq_D_backward[i] = cuu_eq_D_backward_ana[i] = cuu_eq_D_backward_ana1[i] = cuu_eq_D_backward_ana2[i] = cuu_eq_D_backward_ana3[i] = cuu_eq_D_backward_ana4[i] =0;
    
    
    for (w=1; w < n_omega; w++) {
        Rcl0[w] = Rcl_D[w] = GAUSS(&seed) * sigma_x_cl[w];//Classical initial conf sampling
        Vcl0[w] = Vcl_D[w] = GAUSS(&seed) * sigma_p_cl[w];//Classical initial momentum sampling
    }
    ...
    for (j=0; j < MCN; j++) { //Monte Carlo phase-space integration (R,P)
        ...
        for (tau=0; tau<LEN_TRAJ_EQ; tau++) {
            du_accum[tau] =DU(Rcl_D, omega_array, req); 
            MOVEA(Rcl_D, Vcl_D, Fcl_D);
            force_donor(Rcl_D, Fcl_D, omega_array, req);
            MOVEB(Vcl_D, Fcl_D);
        }
        CorrFunc(LEN_TRAJ_EQ, LENMD, du_accum, cuu_eq_D);
        for (i=0; i<LENMD; i++) cuu_eq_D_accum[i] += cuu_eq_D[i];
    }
    
    for (tau=0; tau< LENMD ; tau++)
        for (w=1; w<N; w++) cuu_ana[tau] += omega_array[w]*omega_array[w]*req[w]*req[w]*cos(omega_array[w]*tau*DT)/beta;
    ...
    cout << "Length of Cuu = " << LENMD << endl;

    outfile.open((emptystr+"Cuu_eq_D_"+idstr+".dat").c_str());
    for (i=0; i< LENMD; i++) outfile << cuu_eq_D_accum[i]/MCN << endl;
    outfile.close();
    outfile.clear();
    
    outfile.open((emptystr+"Cuu_ana_"+idstr+".dat").c_str());
    for (i=0; i< LENMD; i++) outfile << cuu_ana[i] << endl;
    outfile.close();
    outfile.clear();
    ...
    for (j=0; j < MCN; j++) { //Monte Carlo phase-space integration (R,P)
        ...
        ducl0_avg += ducl0;
        for (w=1; w < n_omega; w++) duicl0[w] = DUi(Rcl0, omega_array, req, w);
        
        //classical sampling
        force_donor(Rcl_D, Fcl_D, omega_array, req);
        for (tau =0 ; tau< NMD_forward; tau++) {
            //record DU every DT: DU is sum of all frequencies
            du_accum_cl_D[tau] = DU(Rcl_D, omega_array, req);
            
            cuu_eq_D_forward[tau] += ducl0 * du_accum_cl_D[tau];//numerical
            
            propagate_donor(Rcl0, Vcl0, DT*tau, omega_array, Rt);//a bit analytical
            cuu_eq_D_forward_ana1[tau] += ducl0 * DU(Rt, omega_array, req);//nan
            
            for (w=1; w<N; w++) cuu_eq_D_forward_ana2[tau] += duicl0[w] * DUi(Rt, omega_array, req, w);//nan
            
            for (w=1; w<N; w++) cuu_eq_D_forward_ana3[tau] += pow(omega_array[w],4)*req[w]*req[w]*Rcl0[w]*Rcl_D[w];//lower signal bigger oscillation
            
            for (w=1; w<N; w++) cuu_eq_D_forward_ana4[tau] += pow(omega_array[w],4)*req[w]*req[w]*Rcl0[w]*Rcl0[w]*cos(omega_array[w]*tau*DT);
            ...
            if (tau%STEPS == 0) {
                cuu_ana[tau/STEPS] = 0;
                for (w=1; w<N; w++) cuu_ana[tau/STEPS] += omega_array[w]*omega_array[w]*req[w]*req[w]*cos(omega_array[w]*tau*DT)/beta;
        ...
        //classical sampling
        force_donor(Rcl_D, Fcl_D, omega_array, req);
        for (tau =0 ; tau< NMD_backward; tau++) {
            //record DU every DT: DU is sum of all frequencies
            du_accum_cl_D[tau] = DU(Rcl_D, omega_array, req);
            
            cuu_eq_D_backward[tau] += ducl0 * du_accum_cl_D[tau];//numerical
            
            propagate_donor(Rcl0, Vcl0, DT*tau, omega_array, Rt);//a bit analytical
            cuu_eq_D_backward_ana1[tau] += ducl0 * DU(Rt, omega_array, req);
            
            for (w=1; w<N; w++) cuu_eq_D_backward_ana2[tau] += duicl0[w] * DUi(Rt, omega_array, req, w);
            
            for (w=1; w<N; w++) cuu_eq_D_backward_ana3[tau] += pow(omega_array[w],4)*req[w]*req[w]*Rcl0[w]*Rcl_D[w];
            
            for (w=1; w<N; w++) cuu_eq_D_backward_ana4[tau] += pow(omega_array[w],4)*req[w]*req[w]*Rcl0[w]*Rcl0[w]*cos(omega_array[w]*tau*DT);
            
            for (w=1; w<N; w++) cuu_eq_D_backward_ana[tau] += omega_array[w]*omega_array[w]*req[w]*req[w]*cos(omega_array[w]*tau*DT)/beta; //completely analytical
...
/*
    ...
    //2cumu analytical dynamics [1]
    
    
    for (i=0;i< NMD_forward;i++) {
        cuu_eq_D_forward_ana1[i] /= MCN;
        cuu_eq_D_forward_ana1[i] -= ducl0_avg_ana * ducl0_avg_ana;
    }
    for (i=0;i< NMD_backward;i++) {
        cuu_eq_D_backward_ana1[i] /= MCN;
        cuu_eq_D_backward_ana1[i] -= ducl0_avg_ana * ducl0_avg_ana;
    }
    ...
    for (i=0; i< LEN; i++) {
        if (t_array[i] >= 0) {
            MDlen = static_cast<int>(abs(t_array[i] / DT));
            for (j=0; j< MDlen; j++) integrand_marcus[j] = (t_array[i] - j*DT) * cuu_eq_D_forward_ana1[j];
            integral = Integrate(integrand_marcus, MDlen, DT);
            corr1[i] = cos(t_array[i]*ducl0_avg_ana/hbar) * exp(-integral/hbar/hbar);
            corr2[i] = sin(t_array[i]*ducl0_avg_ana/hbar) * exp(-integral/hbar/hbar);
        }
        else {
            MDlen = static_cast<int>(abs(t_array[i] / DT));
            for (j=0; j< MDlen; j++) integrand_marcus[j] = (t_array[i] + j*DT) * cuu_eq_D_backward_ana1[j];
            integral = Integrate(integrand_marcus, MDlen, -DT); //check ABSDT or DT?
            corr1[i] = cos(t_array[i]*ducl0_avg_ana/hbar) * exp(-integral/hbar/hbar);
            corr2[i] = sin(t_array[i]*ducl0_avg_ana/hbar) * exp(-integral/hbar/hbar);
        }
    }
    outfile.open((emptystr+"2cumu_ana1_t_re_"+idstr+".dat").c_str());
    for (i=0; i< LEN; i++) outfile << corr1[i] << endl;
    outfile.close();
    outfile.clear();
    
    outfile.open((emptystr+"2cumu_ana1_t_im_"+idstr+".dat").c_str());
    for (i=0; i< LEN; i++) outfile << corr2[i] << endl;
    outfile.close();
    outfile.clear();
    ...
    outfile.open((emptystr+"2cumu_ana1_re_"+idstr+".dat").c_str());
    for (i=0; i<nn/2; i++) outfile << corr1_orig[i]*LEN*DeltaT*DAcoupling*DAcoupling << endl;
    outfile.close();
    outfile.clear();
    
    outfile.open((emptystr+"2cumu_ana1_im_"+idstr+".dat").c_str());
    for (i=0; i<nn/2; i++) outfile << corr2_orig[i]*LEN*DeltaT*DAcoupling*DAcoupling << endl;
    outfile.close();
    outfile.clear();
    ...
    //2cumu analytical dynamics [2]
    
    
    for (i=0;i< NMD_forward;i++) {
        cuu_eq_D_forward_ana2[i] /= MCN;
        cuu_eq_D_forward_ana2[i] -= ducl0_avg_ana * ducl0_avg_ana;
    }
    for (i=0;i< NMD_backward;i++) {
        cuu_eq_D_backward_ana2[i] /= MCN;
        cuu_eq_D_backward_ana2[i] -= ducl0_avg_ana * ducl0_avg_ana;
    }
    ...
    for (i=0; i< LEN; i++) {
        if (t_array[i] >= 0) {
            MDlen = static_cast<int>(abs(t_array[i] / DT));
            for (j=0; j< MDlen; j++) integrand_marcus[j] = (t_array[i] - j*DT) * cuu_eq_D_forward_ana2[j];
            integral = Integrate(integrand_marcus, MDlen, DT);
            corr1[i] = cos(t_array[i]*ducl0_avg_ana/hbar) * exp(-integral/hbar/hbar);
            corr2[i] = sin(t_array[i]*ducl0_avg_ana/hbar) * exp(-integral/hbar/hbar);
        }
            ...
            MDlen = static_cast<int>(abs(t_array[i] / DT));
            for (j=0; j< MDlen; j++) integrand_marcus[j] = (t_array[i] + j*DT) * cuu_eq_D_backward_ana2[j];
            integral = Integrate(integrand_marcus, MDlen, -DT); //check ABSDT or DT?
            corr1[i] = cos(t_array[i]*ducl0_avg_ana/hbar) * exp(-integral/hbar/hbar);
            corr2[i] = sin(t_array[i]*ducl0_avg_ana/hbar) * exp(-integral/hbar/hbar);
        }
    }
    outfile.open((emptystr+"2cumu_ana2_t_re_"+idstr+".dat").c_str());
    for (i=0; i< LEN; i++) outfile << corr1[i] << endl;
    outfile.close();
    outfile.clear();
    
    outfile.open((emptystr+"2cumu_ana2_t_im_"+idstr+".dat").c_str());
    for (i=0; i< LEN; i++) outfile << corr2[i] << endl;
    outfile.close();
    outfile.clear();
    ...
    outfile.open((emptystr+"2cumu_ana2_re_"+idstr+".dat").c_str());
    for (i=0; i<nn/2; i++) outfile << corr1_orig[i]*LEN*DeltaT*DAcoupling*DAcoupling << endl;
    outfile.close();
    outfile.clear();
    
    outfile.open((emptystr+"2cumu_ana2_im_"+idstr+".dat").c_str());
    for (i=0; i<nn/2; i++) outfile << corr2_orig[i]*LEN*DeltaT*DAcoupling*DAcoupling << endl;
    outfile.close();
    outfile.clear();
    ...
    //2cumu analytical dynamics [3]
    
    
    for (i=0;i< NMD_forward;i++) {
        cuu_eq_D_forward_ana3[i] /= MCN;
        //cuu_eq_D_forward_ana1[i] -= ducl0_avg_ana * ducl0_avg_ana;
    }
    for (i=0;i< NMD_backward;i++) {
        cuu_eq_D_backward_ana3[i] /= MCN;
        //cuu_eq_D_backward_ana1[i] -= ducl0_avg_ana * ducl0_avg_ana;
    }
    ...
    for (i=0; i< LEN; i++) {
        if (t_array[i] >= 0) {
            MDlen = static_cast<int>(abs(t_array[i] / DT));
            for (j=0; j< MDlen; j++) integrand_marcus[j] = (t_array[i] - j*DT) * cuu_eq_D_forward_ana3[j];
            integral = Integrate(integrand_marcus, MDlen, DT);
            corr1[i] = cos(t_array[i]*ducl0_avg_ana/hbar) * exp(-integral/hbar/hbar);
            corr2[i] = sin(t_array[i]*ducl0_avg_ana/hbar) * exp(-integral/hbar/hbar);
        }
            ...
            MDlen = static_cast<int>(abs(t_array[i] / DT));
            for (j=0; j< MDlen; j++) integrand_marcus[j] = (t_array[i] + j*DT) * cuu_eq_D_backward_ana3[j];
            integral = Integrate(integrand_marcus, MDlen, -DT); //check ABSDT or DT?
            corr1[i] = cos(t_array[i]*ducl0_avg_ana/hbar) * exp(-integral/hbar/hbar);
            corr2[i] = sin(t_array[i]*ducl0_avg_ana/hbar) * exp(-integral/hbar/hbar);
        }
    }
    outfile.open((emptystr+"2cumu_ana3_t_re_"+idstr+".dat").c_str());
    for (i=0; i< LEN; i++) outfile << corr1[i] << endl;
    outfile.close();
    outfile.clear();
    
    outfile.open((emptystr+"2cumu_ana3_t_im_"+idstr+".dat").c_str());
    for (i=0; i< LEN; i++) outfile << corr2[i] << endl;
    outfile.close();
    outfile.clear();
    ...
    outfile.open((emptystr+"2cumu_ana3_re_"+idstr+".dat").c_str());
    for (i=0; i<nn/2; i++) outfile << corr1_orig[i]*LEN*DeltaT*DAcoupling*DAcoupling << endl;
    outfile.close();
    outfile.clear();
    
    outfile.open((emptystr+"2cumu_ana3_im_"+idstr+".dat").c_str());
    for (i=0; i<nn/2; i++) outfile << corr2_orig[i]*LEN*DeltaT*DAcoupling*DAcoupling << endl;
    outfile.close();
    outfile.clear();
    ...
    //2cumu analytical dynamics [4]
    
    
    for (i=0;i< NMD_forward;i++) {
        cuu_eq_D_forward_ana4[i] /= MCN;
        //cuu_eq_D_forward_ana1[i] -= ducl0_avg_ana * ducl0_avg_ana;
    }
    for (i=0;i< NMD_backward;i++) {
        cuu_eq_D_backward_ana4[i] /= MCN;
        //cuu_eq_D_backward_ana1[i] -= ducl0_avg_ana * ducl0_avg_ana;
    }
    ...
    for (i=0; i< LEN; i++) {
        if (t_array[i] >= 0) {
            MDlen = static_cast<int>(abs(t_array[i] / DT));
            for (j=0; j< MDlen; j++) integrand_marcus[j] = (t_array[i] - j*DT) * cuu_eq_D_forward_ana4[j];
            integral = Integrate(integrand_marcus, MDlen, DT);
            corr1[i] = cos(t_array[i]*ducl0_avg_ana/hbar) * exp(-integral/hbar/hbar);
            corr2[i] = sin(t_array[i]*ducl0_avg_ana/hbar) * exp(-integral/hbar/hbar);
        }
            ...
            MDlen = static_cast<int>(abs(t_array[i] / DT));
            for (j=0; j< MDlen; j++) integrand_marcus[j] = (t_array[i] + j*DT) * cuu_eq_D_backward_ana4[j];
            integral = Integrate(integrand_marcus, MDlen, -DT); //check ABSDT or DT?
            corr1[i] = cos(t_array[i]*ducl0_avg_ana/hbar) * exp(-integral/hbar/hbar);
            corr2[i] = sin(t_array[i]*ducl0_avg_ana/hbar) * exp(-integral/hbar/hbar);
        }
    }
    outfile.open((emptystr+"2cumu_ana4_t_re_"+idstr+".dat").c_str());
    for (i=0; i< LEN; i++) outfile << corr1[i] << endl;
    outfile.close();
    outfile.clear();
    
    outfile.open((emptystr+"2cumu_ana4_t_im_"+idstr+".dat").c_str());
    for (i=0; i< LEN; i++) outfile << corr2[i] << endl;
    outfile.close();
    outfile.clear();
    ...
    outfile.open((emptystr+"2cumu_ana4_re_"+idstr+".dat").c_str());
    for (i=0; i<nn/2; i++) outfile << corr1_orig[i]*LEN*DeltaT*DAcoupling*DAcoupling << endl;
    outfile.close();
    outfile.clear();
    
    outfile.open((emptystr+"2cumu_ana4_im_"+idstr+".dat").c_str());
    for (i=0; i<nn/2; i++) outfile << corr2_orig[i]*LEN*DeltaT*DAcoupling*DAcoupling << endl;
    outfile.close();
    outfile.clear();
    ...
    //2cumu analytical dynamics most
...
void CorrFunc(const int trun, const int tcor, double A[], double Corr[]){
    //calculate C_AA(t) equilibrium correlation function values, with the trajectory length trun. ###
    //Input: trun numbers, A[0], A[1]=A(dt), A[2]=A(2dt), ... , A[trun]=A(trun dt); where trun=LEN_TRAJ_EQ
    //Output:tcor numbers, Corr[0], Corr[1], Corr[tcor]; where tcor=LEN_TRAJ, also time steps between each corr[i] and corr[i+1] is 1 dt.
    // e.g.  CorrFunc(LEN_TRAJ_EQ, LEN_TRAJ, deltaE_fluc, corr); // deltaE[LEN_TRAJ_EQ], Corr[LEN_TRAJ]
	double norm[tcor];
	int t, t0, tt0, tt0max;
	double A0(0);
