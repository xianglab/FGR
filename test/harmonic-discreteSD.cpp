...
const int Nosc = 200; // # of discrete harmonic oscillators
const double omega_m = 20; // in omega_c , the largest freq sampled in SD
...
int main (int argc, char *argv[]) {
    ...
    int n_omega = static_cast<int>(omega_max / d_omega);// for evenly-distributed sampling form spectral density
    ...
    ////// Discrete SD ------------
    double req_array[Nosc];
    double omega_array[Nosc];
    double Sj[Nosc]; //The Huang-Rhys factor Sj = omega_j * Req_j^2 /2 hbar
    
    //New discretization of Spectral density (Ohmic case)
    cout << "Discrete omega and req and Sj: " << endl;
    double a_wang = Nosc / (eta * (1 - exp(-omega_m)));
    
    for (i = 0; i < Nosc; i++) {
        omega_array[i] = log(a_wang/(a_wang-(i+1)));
        req_array[i] = sqrt(2*hbar / a_wang);
        Sj[i] = omega_array[i] * req_array[i] * req_array[i] * 0.5;
        cout << omega_array[i] << " " << req_array[i] << "  " << Sj[i]<< endl;
    }
    

    //LSC approximation = exact
    ...
    for (i = 0; i < LEN; i++) {
        ...
        integ_im[0] =0;
        for (w = 0; w < Nosc; w++) {
            omega = omega_array[w];//discrete freq
            Integrand_LSC(omega, t, integ_re[w], integ_im[w]);
            integ_re[w] *= Sj[w];
            integ_im[w] *= Sj[w];
        }
        integral_re = SUM(integ_re, Nosc); //Integrate(integ_re, n_omega, d_omega);
        integral_im = SUM(integ_im, Nosc); //Integrate(integ_im, n_omega, d_omega);
    ...
    outfile.open("LSC_newOhm_re.dat");
