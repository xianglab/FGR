...
const double d_omega = 0.1;// for Ohmic 0.025 for gaussian;
const int n_omega = static_cast<int>(omega_max / d_omega);
const int N = n_omega; //number of degrees of freedom

const int LEN = 512;// for ohmic SD  1024 for gaussian SD //number of time steps in FFT

const int STEPS = 1;//50; //for correlation function skip steps
const double DeltaT=0.2;// for ohmic 0.3 for gaussian; //FFT time sampling interval
...
int main (int argc, char *argv[]) {
    ...
    int i,j,k;
    stringstream ss;
    string filename="num_LSC_EFGR_nm_Omega0.2_b1e1_5000_";
    string nameapp;
    string idstr="";
...
void average_error(string filename, int length) {
    ...
    for (j= 0; j< NFILE; j++) {
        ...
        if (!infile.is_open()) cout << "Cannot open file" << endl;
        for (i=0; i< length; i++) {
            infile >> data1[i];
            data1_av[i] += data1[i];
